...
$(function(){
	...
	$('#serchBtn').click(function(){
		location.href="serchActor.jsp?serch=<%=serch%>";
	});
...
<div class="warp">
	...
	<div class="serch_left_box">
		<form name="serchFrm" method="get" action="serchActor.jsp">
		<div class="input-group mb-3">
			 <input type="text" class="form-control" placeholder="배우를 입력하세요" 
			 	aria-label="serchtxt" aria-describedby="button-addon2"  name="serch" value="<%=serch%>">
		...
		<nav aria-label="page">
			<ul class="pagination">
				<%if(pageVo.getFirstPage()>1){%>
						<li class="page-item"><a class="page-link" href="serchActor.jsp?currentPage=<%=pageVo.getFirstPage()-1%>&serch=<%=serch%>">Previous</a></li>	
					<%} %>
				
					<% for(int i = pageVo.getFirstPage(); i <= pageVo.getLastPage(); i++){
						if(i>pageVo.getTotalPage()) break;
						if(i == currentPage){ %>
							<li class="page-item active" aria-current="page"><a class="page-link" href="mserchActor.jsp?currentPage=<%=i%>&serch=<%=serch%>"><%=i %></a></li>
					<%	}else{ %>
							<li class="page-item"><a class="page-link" href="serchActor.jsp?currentPage=<%=i%>&serch=<%=serch%>"><%=i %></a></li>
					<%	}//if %>
					<%}//for %>
					...
					<%if(pageVo.getLastPage()< pageVo.getTotalPage()){%>
						...
						href="serchActor.jsp?currentPage=<%=pageVo.getLastPage()+1%>&serch=<%=serch%>">Next</a></li>
					<%} %>
				</ul>
			</nav>
		</div>				
